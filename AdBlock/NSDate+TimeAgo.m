...
- (NSString *)dateTimeAgo {
    ...
    if (components.year >= 1) {
        return components.year == 1 ? NSDateTimeAgoLocalizedStrings(@"1 year ago") : [self stringFromFormat:@"%%d %@years ago" withValue:components.year];
    } else if (components.month >= 1) {
        return components.month == 1 ? NSDateTimeAgoLocalizedStrings(@"1 month ago") : [self stringFromFormat:@"%%d %@months ago" withValue:components.month];
    } else if (components.weekOfYear >= 1) {
        return components.weekOfYear == 1 ? NSDateTimeAgoLocalizedStrings(@"1 week ago") : [self stringFromFormat:@"%%d %@weeks ago" withValue:components.weekOfYear];
    } else if (components.day >= 1) {    // up to 6 days ago
        return components.day == 1 ? NSDateTimeAgoLocalizedStrings(@"1 day ago") : [self stringFromFormat:@"%%d %@days ago" withValue:components.day];
    } else if (components.hour >= 1) {   // up to 23 hours ago
        return components.hour == 1 ? NSDateTimeAgoLocalizedStrings(@"An hour ago") : [self stringFromFormat:@"%%d %@hours ago" withValue:components.hour];
    } else if (components.minute >= 1) { // up to 59 minutes ago
        return components.minute == 1 ? NSDateTimeAgoLocalizedStrings(@"A minute ago") : [self stringFromFormat:@"%%d %@minutes ago" withValue:components.minute];
...
- (NSString *)dateTimeUntilNow {
    ...
    if (components.hour >= 6) { // if more than 6 hours ago, change precision
        ...
        if (diffDays == 0) { // today!
            ...
            if (startHourComponent.hour < 12 && 12 < endHourComponent.hour) {
